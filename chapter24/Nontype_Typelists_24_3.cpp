...
模板 CTValue（compile time value），然后用它表示类型列表中某种类型的值：
    ...
    class PopFrontT<case1::Typelist<Head, Tail...>> {
    public:
      using Type = case1::Typelist<Tail...>;
    };
    ...
    // 然后结合 MultiplyT，下面的表达式就会返回所有 Primes 中素数的乘积：
    constexpr bool val = Accumulate<Primes, MultiplyT, CTValue<int, 1>>::value;
    
    /*
    不过这一使用 Typelist 和 CTValue 的方式过于复杂，尤其是当所有数值的类型相同的时候。
    可以通过引入 CTTypelist 模板别名来进行优化，它提供了一组包含在 Typelist 中、类型相同的数值：
      */
    template<typename T, T... Values>
    using CTTypelist = Typelist<CTValue<T, Values> ...>;
  ...
  namespace case2 {
    ...
    */
    ...
    // 这样在使用 CTValue 的时候就可以不用每次都去指定一个类型了，从而简化了使用方式：
    using Primes = case1::Typelist<CTValue<2>, CTValue<3>, CTValue<5>, CTValue<7>, CTValue<11>>;
